...
class SyncYandexMetrikaConversionsGoalsCommand extends Command
{
    ...
    protected $description = 'Ночной съём достижений целей из отчёта «Конверсии» Яндекс Метрики';
    ...
    public function handle(
        ...
        foreach ($integrations as $integrationProject) {
            ...
            try {
                ...
                $rows = $metrikaService->fetchConversionsGoalsStats(
                    $dateFrom,
                    $dateTo,
                    $goalIds,
                ...
                $repository->upsertGoalConversions($integrationProject->project_id, $mapped);

                $synced++;
            } catch (Throwable $e) {
                $failed++;
                Log::channel('yandex_metrika')->error('Conversions goals sync failed', [
