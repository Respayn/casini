...
class SyncYandexMetrikaGeoVisitsCommand extends Command
{
    protected $signature = 'metrika:sync-geo-visits';

    protected $description = 'Ночной съём переходов из отчёта «География» Яндекс Метрики';
    ...
    public function handle(
        ...
        foreach ($integrations as $integrationProject) {
            ...
            try {
                ...
                $rows = $metrikaService->fetchGeoVisitsStats(
                    $dateFrom,
                    $dateTo,
                    $visitsMetric,
                ...
                $failed++;
                Log::channel('yandex_metrika')->error('Geography visits sync failed', [
