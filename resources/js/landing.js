...
const eaForm = document.getElementById("earlyAccessForm");
...
// Отправка через SMTP с невидимой SmartCaptcha
let pendingEa = null; // данные формы и элементы управления на время проверки (ранний доступ)
let pendingSubscribe = null; // данные для подписки на новости на время проверки

window.onSmartCaptcha = async function (token) {
    // Ветка 1: ранний доступ
    if (pendingEa) {
        const { form, submitBtn, originalBtnText } = pendingEa;
        try {
            const data = {
                team: form.team.value,
                tg: form.tg.value,
                agency: form.agency.value || "",
                sourceBlock: form.sourceBlock.value || "",
    ...
    // Ветка 2: подписка на новости
    if (pendingSubscribe) {
        const { form, submitBtn, originalBtnText, email } = pendingSubscribe;
        try {
            const response = await fetch("send_email.php", {
                method: "POST",
                headers: {
            ...
            const result = await response.json();
            alert(result.message || (result.success ? "Успешно" : "Ошибка"));
...
eaForm?.addEventListener("submit", (e) => {
    ...
    if (!form.team.value || !form.tg.value) {
    ...
    try {
        if (
            ...
            alert(
                "Не удалось инициализировать проверку безопасности. Обновите страницу."
            );
            submitBtn.disabled = false;
            submitBtn.textContent = originalBtnText;
            pendingEa = null;
...
// Подписка на новости с невидимой SmartCaptcha
