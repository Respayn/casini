...
## [v0.2.39] - 2026-09-21
### 🚀 Новое
- Планирование: галочка «НДС» показывает рекламный бюджет, CPL и CPC с НДС 22% (в базе всегда без НДС); при вводе с галочкой сумма делится на 1,22 перед записью
- Планирование: над полем бюджета / CPL / CPC подсказка «НДС не учитывается» или «учитывая НДС»

### 🔧 Улучшения
- Планирование: при переключении «НДС» показывается скелетон таблицы (как при смене года / «Неактивные…»)
- Планирование: переключатель года выровнен с фильтрами; у цифры года отступы и обычный (не жирный) шрифт

## [v0.2.38] - 2026-09-21
### 🚀 Новое
- Планирование: чекбоксы «Неактивные клиенто-проекты» и «НДС» в фильтрах (как на Каналах / Статистике)
- Планирование: без галочки «Неактивные…» в таблице только активные клиенто-проекты; выбранный в сайдбаре показывается всегда

### 🔧 Улучшения
- Планирование: галочка «НДС» в фильтрах (пересчёт сумм — v0.2.39)

## [v0.2.37] - 2026-09-21
### 🛠 Исправления
- Планирование: убран двойной отступ между переключателем года и таблицей (скелетон без «белых пятен»)
- Планирование: год в переключателе больше не пропадает при загрузке таблицы

### 🔧 Улучшения
- Планирование: смена года без полной перерисовки таблицы (`skipRender` + синхронизация ячеек на клиенте) — ответ Livewire ~14 КБ вместо сотен КБ
- Планирование: оверлей со скелетоном таблицы при смене года (как на Каналах / Статистике)

## [v0.2.36] - 2026-09-21
### 🚀 Новое
- Планирование: дата и кто согласовал квартал (поля `approved_at` / `approved_by`, тултип «Согласовал …»)
- Планирование: модалка «Выйти без сохранения?» при уходе со страницы или смене года с несохранёнными правками

### 🛠 Исправления
- Планирование: кнопка «Сохранить» не активируется при выходе из ячейки без реального изменения значения
- Планирование: для SEO/Трафик снова есть строка «Конверсии»; колонке «Параметр» увеличен min-width

### 🔧 Улучшения
- Планирование: зебра строк, скелетон при смене года, колонка «Тип клиенто-проекта» вместо «Отдел»
- Планирование: главный параметр KPI жирным; лиды, визиты и % позиций — только целые; тултип «Рассчитывается автоматически» для расчётных ячеек
- Планирование: компактный инпут в ячейке без прыжка высоты таблицы

## [v0.2.35] - 2026-09-04
### 🚀 Новое
- Настройки профиля: UX как у клиенто-проекта (кнопки «Сохранить / Отменить» только при изменениях), загрузка/удаление фото, роль в шапке из Spatie

### 🛠 Исправления
- Валидация email в форме пользователя: русское сообщение «некорректный email», кнопка сохранения неактивна при ошибке
- Select статуса: корректная работа с значением `0` / `false`

### 🔧 Улучшения
- Форма пользователя: select статуса учётки (Активен / Неактивен / Подтвердить email); чтение/запись статуса на `User`, enum — справочник UI
- Список пользователей: три бейджа статуса
- Поле «ID пользователя в Мегаплан» убрано с формы; «Обновление сервиса» — disabled (серый ползунок)
- Smoke staging: проверка маршрута `users.edit` и alias `can.access.user.edit`
...
- Выбор клиенто-проекта в сайдбаре фильтрует Каналы, Статистику, Планирование, Отчёты и список клиенто-проектов (session + Livewire-события, подсказка «Сбросить фильтр»)
- Кнопка «Удалить фильтры» (активна при выбранном проекте или поиске); при пустом поиске — «Нет результатов»
- Loading state дерева при поиске и смене роли фильтра: скелетон `x-sidebar.tree-skeleton` (как в Планировании при смене года)

### 🔧 Улучшения
- Автораскрытие дерева, если в списке один сотрудник; сохранение раскрытия пути к выбранному клиенто-проекту после смены фильтра
