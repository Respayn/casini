...
## [v0.2.32] - 2026-08-04
### 🚀 Новое
- Каналы: колонки «Остаток бюджета в Директе» и «Расход в Директе»; расход хранится по дням и суммируется за выбранный период
- Ночной съём расхода Директа (`IntegrationSync` + `yandex_direct_daily_spendings`): полночь по часовому поясу агентства, вчерашний день, очередь с повторными попытками
- Каналы: выбор периода «месяц год — месяц год» (будущие месяцы недоступны); при интервале больше одного месяца в колонке «План» показывается «-»
- Лимит ручных запросов к API Директа: не чаще 1 раза в 5 минут, не более 3 подряд, затем блок 60 минут; ночной съём лимит не тратит

### 🔧 Улучшения
- Остаток бюджета: дата/время актуальности в формате `ЧЧ:мм, дд.мм.гг` по часовому поясу агентства
- Обновление метрик Директа только массовыми действиями (клик по ячейке отключён); сообщение bulk: «Обновлено / ошибок / пропущено»
- Правила workflow: убран обязательный прогон Bugbot перед PR
