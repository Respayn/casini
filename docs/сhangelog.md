...
- Статусы учётки (`UserAccountStatus`): Активен / Неактивен / Подтвердить email; middleware `EnsureUserIsActive`; баннер на входе
- Повторная отправка письма подтверждения email со страницы входа (throttle)
