...
Левая панель: «Поиск по клиентам и клиенто-проектам», «Все клиенты», «Собрать портфель клиенто-проектов по», дерево сотрудник → клиент → клиенто-проект.
...
- повторный клик по тому же проекту, «Все клиенты» или «Сбросить фильтр» в подсказке → сброс;
...
При поиске и смене роли фильтра — loading state как в Планировании при смене года: полупрозрачный оверлей + скелетон дерева (`x-sidebar.tree-skeleton`), поля поиска/фильтра приглушаются (`opacity-60`), список — `opacity-40` (`wire:target="searchQuery,sortBy"`). Поиск/смена роли удерживают ответ ~0.55 с, чтобы скелетон успел отрисоваться (иначе запрос слишком быстрый и оверлей «моргает»).
