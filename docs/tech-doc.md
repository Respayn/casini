...
Операторы: если в тексте есть `*` (как `!*promo*`) — шаблон `=*` / `!*`; если звёздочек нет (как `catalog`) — «содержит» `=@` / `!@`. **Исключение для страницы входа:** значение вида полного URL (`http://…` / `https://…`) без `*` сравнивается точно (`==` / `!=`), как «Полный URL» в Метрике — иначе `!https://site/` означало бы «URL не содержит домен» и обнуляло отчёт.
